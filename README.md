...
- **鱼群**：6 条基于真实斑纹重上色的锦鲤（红白 / 金黄 / 墨斑 / 绯橙 / 白无地），贴近水面巡游、清晰可见，带惯性的转向物理（限角速度/角加速度、过弯减速侧倾、尾频随速度），硬性碰撞分离不穿模，骨骼游动动画；`?debug` 可显示方向箭头校验
...
- **氛围**：固定白天景（绿水通透）；池底细沙焦散；bloom 辉光（桌面端）；程序生成的水声/水滴/风铃音效（无音频资源）
- **中英双语**：默认中文，右上可切英文
